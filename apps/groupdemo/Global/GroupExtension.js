/**
 * a javascript library wrapping helmagroups
 * requires HelmaGroups-0.10
 *
 * $RCSfile$
 * $Revision$
 * $Author$
...
/**
  ...
  */
function HelmaGroupsMgr() {
   var extension = Packages.helma.extensions.helmagroups.GroupExtension.self;
   if (extension == null) {
      throw("HelmaGroupsMgr requires HelmaGroups Java Extension in \
            lib/ext or application directory \
   ...
   /**
    * visible to scripting env: get a group, wrapped as a javascript HelmaGroup object.
   ...
   this.get = function(name) {
      ...
      if (javaGroup == null || javaGroup.isConnected() == false) {
         ...
         return new HelmaGroup(javaGroup);
      }
   };
   ...
   this.checkWriteAccess = function(nameOrJGroup) {
      ...
      if (nameOrJGroup instanceof Packages.helma.extensions.helmagroups.Group) {
         // arg was a Group
   ...
   this.listMembers = function(name) {
      ...
      for (var i=0; i<addrArr.length; i++) {
         arr[arr.length] = Packages.helma.extensions.helmagroups.Config.addressToString(addrArr[i]);
      }
      return arr;
   };
...
// instantiate HelmaGroupsMgr
var groups = new HelmaGroupsMgr();



/**
  * Constructs a new HelmaGroup Object.
  * This is what is retrieved through groups.get(groupName),
  * wrapping the root object of each group tree.
  */
function HelmaGroup(javaGroup) {
   // private variable containing the root GroupObject
   var groupRoot = new GroupObject(javaGroup.getRoot());
   ...
   /**
    * sets a key/value pair on the group's root, wraps the function of the root GroupObject
    */
   ...
   /**
    * @see GroupObject.listProperties
    */   
   ...
   /**
    * @see GroupObject.countProperties
    */   
   ...
   /**
    * calls a function in all connected applications (to be specific: in all registered
    * localClients).
    * @param method name of the method in xmlrpc-style: test is called as root.test(), stories.137.render
    * is called as root.stories.get("137").render() etc etc.
    * @param argArr array of arguments to the remote method
    * @param sendMode as defined for GroupObject
    * @returns array of result objects
    */
   this.callFunction = function(method, argArr, sendMode) {
      groups.checkWriteAccess(javaGroup);
      if (sendMode == null) {
         sendMode = GroupObject.DEFAULT_GET;
      }
      var argVec = new java.util.Vector();
      for (var i=0; i<argArr.length; i++) {
         argVec.add(argArr[i]);
      }
      var resVec = javaGroup.execute(method, argVec, sendMode, javaGroup.DEFAULT_EXECUTE_TIMEOUT);
      var resArr = [];
...
/**
 * Constructs a new GroupObject. This class wraps the java GroupObject
 ...
 */
function GroupObject(javaGroupObject) {

   if (!javaGroupObject) {
      var javaGroupObject = new Packages.helma.extensions.helmagroups.GroupObject();
   }
   ...
   var checkWriteAccess = function() {
      if (javaGroupObject.getState() == Packages.helma.extensions.helmagroups.GroupObject.REPLICATED) {
   ...
   var keyIsPath = function(key) {
      var separator = Packages.helma.extensions.helmagroups.GroupObject.SEPARATOR;
   ...
   this.set = function(key, val, sendMode) {
      if (!key) {
         throw "GroupObject.set(): key can't be null";
      }
      checkWriteAccess();
      ...
      if (val == null)
         ...
         ok = true;
      else if (val instanceof GroupObject)
         ok = true;
      if (ok == false) {   
         throw "only primitive values, Dates and GroupObjects allowed in GroupObject.set()";
      }
      if (sendMode == null) {
         sendMode = GroupObject.DEFAULT_GET;
      }
   ...
   this.get = function(key) {
      ...
      if (keyIsPath(key)) {
         ...
         return new GroupObject(javaGroupObject.getChild(key));
      }
      return null;
   };
   ...
   this.wrap = function(newGroupObject) {
      checkWriteAccess();
      if (javaGroupObject.getState() != Packages.helma.extensions.helmagroups.GroupObject.REPLICATED) {
         throw "GroupObject.wrap() may only be called on replicated GroupObjects";
      }
      if (newGroupObject == null || !(newGroupObject instanceof GroupObject)) {
         throw "GroupObject.wrap() requires a GroupObject as an argument";
      }
   ...
   this.unwrap = function() {
      ...
      javaGroupObjectClone.setState(Packages.helma.extensions.helmagroups.GroupObject.LOCAL);
      javaGroupObjectClone.setPath(null);
      return new GroupObject(javaGroupObjectClone);
   };
   ...
   this.isLocal = function() {
      return (javaGroupObject.getState()
              == Packages.helma.extensions.helmagroups.GroupObject.LOCAL);
   };
